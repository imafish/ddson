...
| 10.1 | [ ]  |         | .   better interfaces to the client to reflect errors and download status            |
| 10.2 | [ ]  |         | .   better interfaces from the agent to reflect errors and download status           |
...
| 20   | [ ]  | Testing | refactor, maybe re-write: Include errors in GRPC response, use *DownloadError        |
| 22   | [ ]  | Testing | a test plan                                                                          |
| 22.1 | [ ]  | Testing | .   UT                                                                               |
| 22.2 | [ ]  | Testing | .   GRPC interface test                                                              |
| 22.3 | [ ]  | Testing | .   Integration test (with a python server as download source)                       |
| 23   | [ ]  | Testing | pass the http credentials along with download requests                               |
| 24   | [ ]  | ^^^     | BUG: fix log output -- if using logger, no dim and log type is wrong                 |
| 25   | [ ]  | ^^^     | BUG: File is not being transferred after completing downloading                      |
| 26   | [ ]  | ^^^     | Do not drop and ban agent if heartbeat timeout                                       |
| 27   | [ ]  | ^^^     | BUG: Do not drop and ban agent in some timeout situation, like TLS handshake timeout |
| 28   | [ ]  | ^^^     | BUG: error is reported even if download is successful                                |
